...
import { DisabledButton } from "components/lending/reactiveButton";
  ...
  return (
    <Container>
      <img
        ...
        alt="canto"
      />
      <h2>{token.data.underlying_name}</h2>

      <h2>{
        (token.borrowBalance > 0) ? `you cannot uncollateralize an asset that is currently being borrowed. Please repay all ${token.data.underlying_name.toLowerCase()} before uncollateralizing.` : (ifLimit() && props.decollateralize) ? "80% of your borrow limit will be used. please repay borrow's or increase supply." :    
      ...
      >
        {
          (token.borrowBalance > 0 || ifLimit()) && props.decollateralize ? <DisabledButton>
            {token.borrowBalance > 0 ? `currently borrowing ${token.data.underlying_symbol.toLowerCase()}` : `80% borrow limit will be reached`}
        ...
        >
          {props.decollateralize
            ? `disable ${token.data.underlying_symbol.toLowerCase()} as collateral`
            : `use ${token.data.underlying_symbol.toLowerCase()} as collateral`}
        </Button>
        }
